...
def Gauss_Jacobi(matriz, vetor_b, iteracoes_Gauss_Jacobi, epsilon_Gauss_Jacobi):
	...
	while loop:
		...
		for i in range(linhas):
			...
			vetor_x1[i] = (somatorio + vetor_b[i]) / matriz[i][i]
			
		for i in range(linhas): 
			if abs(vetor_x1[i] - x0[i]) < epsilon_Gauss_Jacobi:
				loop = False
...
Gauss_Jacobi(matriz, vetor_b, 20, 1e-5)
